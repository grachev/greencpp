...
public:
    ...
    inline float SecondsFloat() throw () {
        return Value_ / 1000000.0f;
    }
...
public:
    ...
    static TDuration Parse(const Stroka&);
...
public:
    ...
    static TInstant ParseIso8601(const Stroka&);
    static TInstant ParseRfc822(const Stroka&);
    static TInstant ParseHttp(const Stroka&);
    static TInstant ParseX509Validity(const Stroka&);
...
inline TDuration operator- (const TInstant& l, const TInstant& r) throw () {
    return TDuration(l.GetValue() - r.GetValue());
}
...
inline TInstant operator+ (const TInstant& i, const TDuration& d) throw () {
    ...
    return TInstant(i.GetValue() + d.GetValue());
}
